...
createApp({
    setup() {
        ...
        function update_remaining_time() {
            ...
            remaining_time.value = Math.max(total_duration.value - elapsed, 0) / 1000;
        }
        ...
        function stage_percent_start(index) {
            ...
            const sum_before = stages.value.slice(0, index).reduce((sum, s) => sum + s.duration, 0);
            return (sum_before / total_duration.value) * 100;
        }

        function stage_percent_end(index) {
            if (!total_duration.value) return 100;
            const sum_to = stages.value.slice(0, index + 1).reduce((sum, s) => sum + s.duration, 0);
        ...
        function handle_message(event) {
            ...
            if (action === 'START_PROGRESS') {
                ...
                if (data.result === true && current_stage_index.value === data.stage_index - 1) {
                    skip_current_stage();
                } else {
